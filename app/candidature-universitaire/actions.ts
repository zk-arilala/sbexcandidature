...
import {
  candidatureUniversitaireSchema,
  CandidatureUniversitaireInput,
} from "./schema"
...
export async function creerCandidatureUniversitaire(
  data: CandidatureUniversitaireInput& {
  ...
  const parsed = candidatureUniversitaireSchema.safeParse(data)
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      console.log("Données CANDIDAT envoyées à Prisma:", {
        ...
        pere_nom: data.pere_nom,
        pere_profession: data.pere_profession,
        mere_nom: data.mere_nom,
        mere_profession: data.mere_profession,
        ...
        nombre_frere_soeur: data.nombre_frere_soeur,
        noms_frere_soeur: data.noms_frere_soeur,
        dernier_diplome_id: 1,
        dernier_diplome_annee: data.annee_obtention,
      });
      ...
      currentNumeroDossier = `BU-${currentYear}-${currentDemande_id}`;
      ...
      /*const tempPath = path.join(process.cwd(), "public/pieces_jointes", "BU_TEMP_0000");
      const truePath = path.join(process.cwd(), "public/pieces_jointes", demandeUpdated.numero_dossier);
      try {
        await fs.access(tempPath); 
        await fs.rename(tempPath, truePath);
      } catch (err) {
          console.error("Erreur lors du renommage des fichiers :", err);
      }*/
